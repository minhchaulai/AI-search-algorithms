...
def backtracking_search(csp, lcv, mcv):
    if lcv == False and mcv == False:
        return recursive_backtracking(OrderedDict(), csp)
    if mcv and lcv == False:
        return recursive_backtracking_mcv(OrderedDict(), csp)
    if mcv == False and lcv:
        return recursive_backtracking_lcv(OrderedDict(), csp)
    else:
        return recursive_backtracking_both(OrderedDict(), csp)
...
def recursive_backtracking_lcv(assignment, csp):
    if len(assignment) == len(csp.states):
        if constraint(assignment, canada):
            return (assignment, csp.backs)
        else:
    ...
    for neighbor in csp.neighbors[state]:
        if neighbor in assignment:
            previousColors.append(assignment[neighbor])
    for color in csp.colors:
        if color not in previousColors:
            if valid(assignment, state, color):
                assignment[state] = color
                csp.current += 1
                result = recursive_backtracking(assignment, csp)
...
def recursive_backtracking_both(assignment, csp):
    if len(assignment) == len(csp.states):
        if constraint(assignment, canada):
            return (assignment, csp.backs)
        else:
